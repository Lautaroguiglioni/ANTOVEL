...
server.tool(
  "search_memories",
  "Search the user's memory bank using semantic similarity. Returns the most relevant memories for a natural language query like 'viajes con familia' or 'recuerdos de la infancia'.",
  {
...
server.tool(
  "get_essence",
  "Retrieve the Essence Document for a patient — their identity affirmation, key people, life timeline, daily anchors, and reminiscence prompts. Used by caregivers and the AI assistant.",
  {
...
server.tool(
  "add_memory",
  "Persist a new memory to the brain. Automatically generates a semantic embedding for future search. Use this when the user uploads a photo, records audio, or writes a note.",
  {
    userId: z.string().describe("User/patient ID"),
...
server.tool(
  "daily_prompt",
  "Generate a therapeutic daily prompt for Alzheimer memory training. Selects the most relevant memory based on training history and the patient's current emotional state. Lights up the corresponding brain node.",
  {
    patientId: z.string().describe("Patient ID"),
    currentState: z
...
server.tool(
  "train_from_answer",
  "Record a patient's response to a training prompt. Tracks whether they recalled the memory, their confidence level, and generates a recommendation for future training frequency.",
  {

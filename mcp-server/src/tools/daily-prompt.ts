...
const PROMPT_TEMPLATES: Record<string, (m: any) => string> = {
  identity: () => "¿Cómo te llamás vos? ¿Cuál es tu nombre completo?",
  family_bond: (m) =>
    `¿Te acordás de ${m.donor_name || "alguien de tu familia"}? ¿Quién es?`,
  life_milestone: (m) =>
    `¿Recordás algo importante que pasó en el año ${new Date(m.date).getFullYear()}?`,
  happy_place: () =>
    "¿Cuál es un lugar que siempre te gustó? ¿Un lugar que te da paz?",
  daily_anchor: () =>
    "¿Qué es lo primero que hacés cuando te levantás por la mañana?",
  sensory: () =>
    "¿Hay alguna música o aroma que te recuerde algo especial?",
}

function getDefaultPrompt(): DailyPromptResult {
  return {
...
export function generateDailyPrompt(
  ...
  if (currentState === "agitated" || currentState === "confused") {
    ...
    const safe = leastRecalled.find(
      (m) => m.therapeutic_tag === "identity" || m.therapeutic_tag === "daily_anchor",
    )
    if (safe) selected = safe
  } else if (currentState === "happy") {
    // When happy: try happy places or life milestones
    const joyful = leastRecalled.find(
      (m) => m.therapeutic_tag === "happy_place" || m.therapeutic_tag === "life_milestone",
    )
    if (joyful) selected = joyful
  }
